-- ==================== MISE À JOUR DES TABLES CHALLENGES ====================
-- Ce script met à jour les tables existantes pour utiliser les points au lieu des niveaux

-- Supprimer les anciennes policies
DROP POLICY IF EXISTS "Les challenges sont visibles par tous" ON challenges;
DROP POLICY IF EXISTS "Les validations sont visibles par tous" ON challenge_validations;
DROP POLICY IF EXISTS "Tout le monde peut créer des challenges" ON challenges;
DROP POLICY IF EXISTS "Tout le monde peut supprimer des challenges" ON challenges;
DROP POLICY IF EXISTS "Tout le monde peut valider des challenges" ON challenge_validations;
DROP POLICY IF EXISTS "Tout le monde peut supprimer des validations" ON challenge_validations;
...
CREATE INDEX idx_challenge_validations_user_email ON challenge_validations(user_email);
...
-- ('50', 'Boire 2L d''eau', 'Bois 2 litres d''eau dans la journée', 50),
-- ('150', 'Faire 100 pompes', 'Fais 100 pompes dans la journée (tu peux les répartir)', 150),
-- ('300', 'Courir 10km', 'Cours 10 kilomètres d''affilée', 300);
